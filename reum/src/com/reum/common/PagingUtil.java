...
public class PagingUtil {
	...
	public PagingUtil(String url, int currentPage, int totRecord, int blockCount, int blockPage) {
		...
		maxPage = (int) Math.ceil((double) totRecord / blockCount);  //최대페이지에서 블록 수만큼 나눈값      9/6 = 1
		if (maxPage == 0) {
			maxPage = 1;
		}
		...
		startSeq = (currentPage - 1) * blockCount + 1;   //1, 6, 11, 16, 21
		endSeq = currentPage * blockCount;				 //5, 10,15, 20, 25
